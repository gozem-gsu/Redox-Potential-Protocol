...
# Loop through folders '117' to '182'

for folder in [f"{i:03d}" for i in range(181,183)]:
    for subdir in ["neutral", "radical", "anion"]:
        ...
        opt_xyz_file_path = os.path.join(
            folder,
            subdir,
            f"{folder}-an.coordinates"
            if subdir == "anion"
            else f"{folder}-rad.coordinates"
            if subdir == "radical"
            else f"{folder}.coordinates",
        )
            ...
            if not check_negative_frequencies(log_file_path):
                ...
                for i in range(len(extracted_calculation_coordinates)):
                    extracted_coordinates.append(
                        ...
                        + [
                            float(extracted_calculation_coordinates[i][1]) * 0.2
                            + float(extracted_primary_coordinates[i][1]),
                            float(extracted_calculation_coordinates[i][2]) * 0.2
                            + float(extracted_primary_coordinates[i][2]),
                            float(extracted_calculation_coordinates[i][3]) * 0.2

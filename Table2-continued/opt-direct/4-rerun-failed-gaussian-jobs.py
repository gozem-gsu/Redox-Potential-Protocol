...
for folder in [f"{i:03d}" for i in range(10)]:
    for subdir in ["neutral", "radical", "anion"]:
        ...
        com_file_name = (
            f"{folder}-an.com"
            if subdir == "anion"
            else f"{folder}-rad.com"
            if subdir == "radical"
            else f"{folder}.com"
        )

        log_file_path = os.path.join(current_opt, folder, subdir, log_file_name)
        com_file_path = os.path.join(current_opt, folder, subdir, com_file_name)
        ...
        if os.path.exists(log_file_path):
            ...
            if not check_termination(log_file_path):
                print(
                    f"Warning: 'Normal termination of Gaussian 16' not found in {log_file_path}"
                )
                ...
                if os.path.exists(com_file_path):
                    ...
                    subprocess.run(
                        ["sbatch", "gaussian.sh"],
                        cwd=os.path.join(current_opt, folder, subdir),
                    )

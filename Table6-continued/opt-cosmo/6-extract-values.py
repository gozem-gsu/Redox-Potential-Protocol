...
def extract_total_energy(out_file_path):
    ...
    with open(out_file_path, 'r') as file:
        for line in file:
            if 'SCF   energy in the final basis set' in line:
    ...
    for i in range(292,317):
        ...
        with open(csv_file_path, 'w', newline='') as csvfile:
            ...
            for subdir in ['radical', 'neutral', 'anion']:
                ...
                COSMO_out_file_name = f"{folder}-an-COSMO.out" if subdir == 'anion' else f"{folder}-rad-COSMO.out" if subdir == 'radical' else f"{folder}-COSMO.out" 
                COSMO_out_file_path = os.path.join(folder, subdir, COSMO_out_file_name)
                ...
                if os.path.exists(log_file_path):
                    ...
                    COSMO_scf_done_value = extract_total_energy(COSMO_out_file_path)

                    writer.writerow([subdir, scf_done_value, thermal_correction_value, COSMO_scf_done_value])
                else:

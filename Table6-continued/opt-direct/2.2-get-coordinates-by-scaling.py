...
def extract_coordinates(log_file):
    ...
    if last_standard_orientation is not None:
        for i in range(last_standard_orientation, len(log_content)):
            if "Rotational constants (GHZ)" in log_content[i] or "Distance matrix (angstroms)" in log_content[i] :
                end_index = i
...
# Loop through folders '304' to '317'

for folder in [f"{i:03d}" for i in range(297,298)]:

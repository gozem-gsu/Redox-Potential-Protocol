        ...
        for row in reader:
            if row['Protonation State'] == 'anion':
                ...
                if thermal_correction and scfe_g:
                    deltaG_Q2_minus = float(row['Thermal Correction to Gibss Free Energy']) + float(row['SCFE(g)'])
                    deltaGsol_Q2_minus = float(row['SCFE(sol)']) - float(row['SCFE(g)'])

            elif row['Protonation State'] == 'radical':
                scfe_g = row['SCFE(g)']
                if scfe_g:
                    deltaG_Q_minus= float(row['SCFE(g)']) + float(row['Thermal Correction to Gibss Free Energy'])
                    deltaGsol_Q_minus= float(row['SCFE(sol)']) - float(row['SCFE(g)'])

        if deltaG_Q_minus is not None and deltaG_Q2_minus is not None:
            deltaGred_g = deltaG_Q2_minus - deltaG_Q_minus

        if deltaGsol_Q2_minus is not None and deltaGsol_Q_minus is not None and deltaGred_g is not None:

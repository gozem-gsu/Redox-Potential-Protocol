    ...
    for i in range(292,317):
        ...
        with open(csv_file_path, 'w', newline='') as csvfile:
            ...
            for subdir in ['radical', 'neutral', 'anion']:
                ...
                PCM_log_file_name = f"{folder}-an-PCM.log" if subdir == 'anion' else f"{folder}-rad-PCM.log" if subdir == 'radical' else f"{folder}-PCM.log" 
                PCM_log_file_path = os.path.join(folder, subdir, PCM_log_file_name)
                ...
                if os.path.exists(log_file_path):
                    ...
                    pcm_scf_done_value = extract_last_scf_done(PCM_log_file_path)

                    writer.writerow([subdir, scf_done_value, thermal_correction_value, pcm_scf_done_value])
                else:

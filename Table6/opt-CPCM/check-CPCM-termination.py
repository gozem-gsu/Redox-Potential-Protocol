...
for folder in [f"{i:03d}" for i in range(227,292)]:
    for subdir in ['neutral', 'radical', 'anion']:
        out_file_name = f"{folder}-an-CPCM.out" if subdir == 'anion' else f"{folder}-rad-CPCM.out" if subdir == 'radical' else f"{folder}-CPCM.out"

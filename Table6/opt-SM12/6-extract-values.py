    ...
    for i in range(227,292):
        ...
        with open(csv_file_path, 'w', newline='') as csvfile:
            ...
            for subdir in ['radical', 'neutral', 'anion']:
                ...
                SM12_out_file_name = f"{folder}-an-SM12.out" if subdir == 'anion' else f"{folder}-rad-SM12.out" if subdir == 'radical' else f"{folder}-SM12.out" 
                SM12_out_file_path = os.path.join(folder, subdir, SM12_out_file_name)
                ...
                if os.path.exists(log_file_path):
                    ...
                    SM12_scf_done_value = extract_total_energy(SM12_out_file_path)

                    writer.writerow([subdir, scf_done_value, thermal_correction_value, SM12_scf_done_value])
                else:
